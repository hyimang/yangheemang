...
$(document).ready(function(){
	...
	var current = location.pathname;//current변수저장소에서 board, mamber 클릭한 내용 확인
	...
	$(".nav-treeview li a").each(function(){
		...
		if($(this).attr('href').indexOf(current_2) != -1) {//위 문제를 처리하는 대체함수 indexOf 사용
...
//each함수로 a태그 찾는 명령. 결과는 2개이기때문에 2번반복
//$(this) 현재 함수의 구현 대상 본인을 가르킴
//2개의 값을 비교1: a태그의 값 /admin/member_list, /admin/board_list
//2개의 값을 비교2: 비교대상 current_2(현재웹브라우저의 URL값중 제일 마지막 값)
//if($(this).attr('href').includes(current_2) == true) {//includes 포함하고있는 문자열을 비교
	//위에 사용한 includes라는 함수는 크롬에서만 작동되어서 IE에선 작동이 안됨 ->크로스브라우징 처리를 해줘야함
</script>
</body>
</html>
<!-- 관리자단 푸터  끝 -->
