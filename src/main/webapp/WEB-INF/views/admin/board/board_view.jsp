              ...
              <div class="card-body">
                ...
                <c:forEach var="index" begin="0" end="1"> 
	                <c:if test="${boardVO.save_file_names[index] != null}">
		                ...
		                <c:choose>
		                	<c:when test="${fn:containsIgnoreCase(checkImgArray, extName) }"><!-- true이면 이미지 파일이란 의미 -->
		                	...
		                	<!-- 사용자홈페이지 메인 최근 게시물 미리보기 이미자가 없을때 사용예정 -->
		                </c:otherwise>
		                </c:choose>
		               </p>
	                </c:if>
...
var printReplyList = function(data, target, templateObject) {
	...
	//target.after(html);//target은 .time-label 클래스영역을 가리킵니다. after 외부내용 추가
	//target.append(html);//target은 #div_reply 아이디 영역을 가리킵니다. append 내부내용추가 기존내용에 뒤붙
	target.prepend(html);//prepend 내부내용 추가시 기존내용의 앞에 추가합니다.
};
</script>
<!-- btn_reply_list 버튼에 적용한 ajax로 댓글 리스트를 구하는 함수를 외부로 뺍니다. -->
<!-- 외부로 함수를 빼는 이유는 btn_reply_list버튼에 토글기능을 적용돼서, 토글기능과 Ajax 기능을 분리하는 목적 -->
<script>
...
<script>
//위 댓글 페이징에서 링크 태그의 이동을 방지하고, btn_reply_list버튼을 클릭해서
// /reply/reply_list/${boardVO.bno}/{1}-> 링크한 페이지값으로 대체해서 실행의 역할을 하는 코드(아래)
$(document).ready(function(){
	$(".pagination").on("click", "li a", function(){
		event.preventDefault();//a태그의 기본기능인 이동기능을 막겠다는 명령
		var page = $(this).attr("href");//현재 클릭한 페이지값을
		//alert(page);//디버그
		$("#reply_page").val(page);
		//$("#btn_reply_list").click();//페이징번호에서 해당되는 번호를 클릭했을때, btn_reply_list버튼을클릭
		//위 버튼을 클릭하면 토글기능이 작동되기 때문에 댓글목록만 가져오는 replyList 함수를 실행(아래)
		replyList();
	});
});
</script>
...
$(document).ready(function() {
	$("#insertReplyBtn").on("click", function() {//댓글등록버튼을 클릭했을 때 구현내용(아래)
		...
		var bno = "${boardVO.bno}";//자바EL변수값
		var reply_text = $("#reply_text").val();//input 태그값
		var replyer = $("#replyer").val();//댓글폼 input태그값
		//alert(bno + ":" + reply_text + ":" + replyer);//디버그
		//return false;//디버그용 강제중지
		if(reply_text == "" || replyer == ""){//불린boolean타입
			//불린boolean타입( && = and 둘다 true면 모두 true) 
			//불린boolean타입 (조건문 || = or 둘중 하나라도 true면 모두 true)
