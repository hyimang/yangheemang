...
public class BoardServiceImpl implements IF_BoardService {
	...
	@Transactional//에러발생시 업데이트 부분이 롤백이 됩니다.
	@Override//아래 메서드는 구현이 2개 있는데, 조회수 업데이트 정상처리 후 조회에러 발생 시 조회수를 원상복귀시키는 작업이 트랜잭션 처
	...
	public void insertBoard(BoardVO boardVO) throws Exception {
		...
		if(save_file_names == null) { return; }//배열첨부파일이 없으면 진행 빠져나
		for(String save_file_name:save_file_names) {//첨부파일 1개일때는 1번만 반복됩니다.
			if(save_file_name != null) {//첨부파일배열에서 배열값이 잇는 경우만
	...
	public void updateBoard(BoardVO boardVO) throws Exception {
		...
		for(String save_file_name:save_file_names) {//첨부파일 갯수만큼 반복됩니다.
