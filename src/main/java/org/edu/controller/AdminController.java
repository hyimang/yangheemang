...
public class AdminController {
	//@Inject == @Autowired 의존성 주입방식(Dependency Inject)으로 
	//외부라이브러리 =모듈=실행클래스= 인스턴스 갖다쓰기
	@Inject
	...
	public String board_wrtie(MultipartFile file, BoardVO boardVO) throws Exception {
		//post 받은 boardVO내용을 DB서비스에 입력하면 됨
		//DB에 입력 후 새로고침 명령으로 게시물테러를 당하지 않으려면, redirect로 이동처리함
	...
	public String board_view(@RequestParam("bno") Integer bno, Model model) throws Exception {
		...
		//실제로 아래처럼 더미데이털르 만드는 것이 아닌 쿼리스트링으로 bno(게시물 고유번호)를 이용해서
		//DB에서 select * from tbl_board where bno = ? 마이바티스 실행된 결과값이 BoardVO형으로 받아서 jsp로 보내줌
		BoardVO boardVO = new BoardVO();
		boardVO.setBno(1);
		boardVO.setTitle("첫번째 게시물입니다.");
		String xss_data = "첫번쨰 내용입니다.<br><br>줄바꿈처리입니다. <script>location.href('http://naver.com');</script>";
		boardVO.setContent(securityCode.unscript(xss_data));//unscript를 통해 위에 사이트 이동을 막음!
		...
		return "admin/board/board_view";//suffix 생략
	}
	...
	public String board_list(Model model) throws Exception {//throws Exception: 스프링으로 예외처리할거 보내기
		...
		input_board2.setTitle("두번째 게시물입니다.");
		input_board2.setContent("두번째 내용입니다.<br>줄바꿈했습니다.");
	...
	public String member_update(PageVO pageVO, MemberVO memberVO) throws Exception {
		...
		//redirect를 사용하는 목적은 새로고침 했을때, 위updateMember메서드 재실행방지 목적
	...
	//현재컨트롤러 클래스에서 member_view.jsp로 데이터를 보내는 역할 Model클래스 사용
	...
	public String member_view(@ModelAttribute("pageVO") PageVO pageVO, @RequestParam("user_id") String user_id, Model model) throws Exception {
		//위에서 수신한 user_id를 개발자가 만든  user_id2이름으로 member_view.jsp 보냅니다
		//member_view.jsp에서 model로 수신한 데이터 user_id2를 출력하는 방법
		MemberVO memberVO = memberService.readMember(user_id);
		model.addAttribute("memberVO", memberVO);
	...
	public String member_list(@ModelAttribute("pageVO")PageVO pageVO, Model model) throws Exception {
		 ...
		 * //{"user_id":"admin","user_name":"관리자",...} 해시데이터<키,값> //Map 타입이 부모,
		 * HashMap타입 자식클래스. paramMap오브젝트의 확장하기 편하도록 위해서 Map<String,Integer> mapTest =
		 * new HashMap<String, Integer>(); String ageValue = "40"; int ageValue2 = 40;
		 ...
		 * paramMap.put("age", 40); System.out.println("해시데이터 타입출력" + paramMap);
		 * 
		 * //members 2차원배열 변수를 MemberVo 클래스형 오브젝트로 members_array 변경 MemberVO
		 ...
		 * Date toDay = new Date(); members_input.setReg_date(toDay);//데이터 타입이 Date형이기
		 * 때문에 java의 날짜데이터를 입력 members_input.setLevels("ROLE_ADMIN");
		 * members_input.setPoint(10);//point 데이터타입이 Interger형이기 때문에 숫자만 입력 //위
		 * members_input 오브젝트에는 1개의 라인만 입력되어 있어서, 이 오브젝트를 배열오브젝트에 저장 MemberVO[]
		 * members_array = new MemberVO[2];//클래스형 배열 오브젝트 생성/ [2]는 레코드 갯수
		 * members_array[0] = members_input; members_array[1] = members_input;
		 * //-------------------------------------------------------------------
		 * //배열타입으로 보내지 않고, List타입으로 model이용해서 jsp로 보냅니다. List<MemberVO> members_list =
		 * Arrays.asList(members_array); //위에서 만든 members_array 배열오브젝트를
		...
		pageVO.setTotalCount(countMember);//전체회원의 수를 구한 변수값 매개변수로 입력하는 순간 calcPage()메서드 실행
		...
		//만약에 위 @ModelAttribute를 사용한다면, 아래 문장이 없어도 됨.
