...
- 댓글에서 GET으로 전송하는 방식2(시만텍웹방식):"/reply/reply_list/" +bno,
- 시만텍웹방식으로 전송하는 목적은 구글 검색에 노출되기 쉽게하기 위해서 검색순위를 올리기 위해서(검색광고와 관련)
- 시만텍웹(의미있는 웹페이지, 주소를 만들자!): html5기술, url쿼리스트링에서 ?를 뺴고, 값만 보내기
- 특이사항: RestAPI에서 select는 GET이 기본이지만 POST로 설정
- 부메랑(크롬의 프러그인): JUnit(스프링-자바단위 테스트=CRUD)와 같은 역할. RestAPI의 단위 테스트(CRUD테스트)를 할 때 사용.

#### 20210104(월)
- Json 데이터는 문자열기반이고, 구조가 Key:Value형식으로 구성되어 있음
